...
export function deutschOracle(f: (x: number) => number): Circuit {
  return {
    n: 2,
    ops: [
      ...
      ...(f(0) !== f(1)
        ? [{ type: 'gate' as const, name: 'Z', qubits: [0] }]
        : []),
      { type: 'gate', name: 'H', qubits: [0] },
    ],
  };
...
export function runDeutsch(f: (x: number) => number): { constant: boolean; measured: string } {
...
export function runDeutschJozsa(n: number, f: (x: number) => number): { balanced: boolean } {
  const dim = 1 << (n + 1);
  ...
  for (let q = 0; q <= n; q++) {
    ...
    for (let i = 0; i < dim; i++) {
      for (let b = 0; b < 2; b++) {
        ...
        if (bit !== b) newAmps[newI] = C.scale(-1, newAmps[newI]);
      }
    }
    amps = newAmps;
  }
  ...
  for (let x = 0; x < (1 << n); x++) {
    if (f(x) === 1) {
      for (let i = 0; i < dim; i++) {
        if ((i & xMask) === x) amps[i] = C.scale(-1, amps[i]);
      }
    }
  }
  // H on first n qubits
  for (let q = 0; q < n; q++) {
    const newAmps = [...amps];
    for (let i = 0; i < dim; i++) newAmps[i] = C.zero();
    for (let i = 0; i < dim; i++) {
      for (let b = 0; b < 2; b++) {
        const other = i & ~(1 << (n - q));
        const newI = other | (b << (n - q));
...
export function runBernsteinVazirani(n: number, secret: number[]): number[] {
  const f = (x: number) =>
    secret.reduce((acc, s, i) => acc ^ (s * ((x >> (n - 1 - i)) & 1)), 0);
  const result = runDeutschJozsa(n, f);
  // After DJ, measuring gives secret - simplified: compute via simulation
  const dim = 1 << n;
  let amps = Array.from({ length: dim }, () => C.zero());
  for (let i = 0; i < dim; i++) amps[i] = C.scale(SQ2, C.one());
  for (let x = 0; x < dim; x++) {
    if (f(x) === 1) amps[x] = C.scale(-1, amps[x]);
  }
  for (let q = 0; q < n; q++) {
...
export function groverStep(state: StateVector, marked: number): StateVector {
  ...
  const amps = state.amplitudes.map((a, i) =>
    i === marked ? C.scale(-1, a) : a
  );
  // Diffusion
...
export function runGrover(n: number, marked: number, iterations?: number): StateVector {
  const dim = 1 << n;
...
/** QFT on amplitudes. */
export function qft(state: StateVector): StateVector {
  const n = state.n;
  const N = 1 << n;
  const input = state.amplitudes;
  const output: typeof input = Array.from({ length: N }, () => C.zero());
  for (let k = 0; k < N; k++) {
    for (let x = 0; x < N; x++) {
...
/** Inverse QFT. */
export function iqft(state: StateVector): StateVector {
  const n = state.n;
  const N = 1 << n;
  const input = state.amplitudes;
  const output: typeof input = Array.from({ length: N }, () => C.zero());
...
export function phaseEstimation(
...
): { estimate: number; binary: string; error: number } {
  const m = precisionQubits;
  const n = m + 1;
  let state = zeroState(n);
  // Eigenstate on last qubit |1>
  state = fromAmplitudes(n, state.amplitudes.map((a, i) =>
    (i & 1) === 1 ? C.one() : C.zero()
  ));
  ...
  const controlState = fromAmplitudes(
    m,
    state.amplitudes.slice(0, 1 << m).map((a) => ({ ...a }))
  );
...
export function modExp(a: number, x: number, N: number): number {
  let result = 1;
  let base = a % N;
  let exp = x;
  while (exp > 0) {
...
export function findPeriod(a: number, N: number): number {
  ...
  for (let x = 0; x < N * N; x++) {
    val = (val * a) % N;
    if (seen.has(val)) return x + 1 - (seen.get(val) ?? 0);
    seen.set(val, x + 1);
  }
  return 1;
}
...
export function shorFactor(N: number, a?: number): { p: number; q: number; r: number; a: number } | null {
  const base = a ?? 2;
  if (gcd(base, N) > 1) return { p: gcd(base, N), q: N / gcd(base, N), r: 1, a: base };
  const r = findPeriod(base, N);
  if (r % 2 !== 0) return null;
  const half = modExp(base, r / 2, N);
  if ((half + 1) % N === 0) return null;
  const p = gcd(half - 1, N);
  const q = gcd(half + 1, N);
  if (p <= 1 || q <= 1 || p * q !== N) return null;
  return { p, q, r, a: base };
}

/** CHSH correlation simulation. */
export function chshExperiment(
  a: number, b: number, ap: number, bp: number,
  trials: number
): { E: Record<string, number>; S: number } {
  const corr = (angleA: number, angleB: number) => {
    let sum = 0;
...
export function runBB84(numQubits: number, eveEnabled = false): BB84Result {
  const aliceBits = Array.from({ length: numQubits }, () => Math.random() < 0.5 ? 0 : 1);
  const aliceBases = Array.from({ length: numQubits }, () => (Math.random() < 0.5 ? 'Z' : 'X') as 'Z' | 'X');
  const bobBases = Array.from({ length: numQubits }, () => (Math.random() < 0.5 ? 'Z' : 'X') as 'Z' | 'X');
  const bobResults: number[] = [];
  for (let i = 0; i < numQubits; i++) {
    let bit = aliceBits[i];
    if (aliceBases[i] === 'X') bit = bit; // encoded as +/-
    let measured = bit;
    if (eveEnabled && Math.random() < 0.5) {
      const eveBasis = Math.random() < 0.5 ? 'Z' : 'X';
      if (eveBasis !== aliceBases[i]) measured = Math.random() < 0.5 ? 0 : 1;
    }
...
export function bitFlipSyndrome(errorQubit: number | null): [number, number] {
  if (errorQubit === null) return [0, 0];
  if (errorQubit === 0) return [1, 1];
  if (errorQubit === 1) return [1, 0];
  return [0, 1];
}
