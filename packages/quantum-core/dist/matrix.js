...
function createMatrix(rows, cols, data) {
    return {
        ...
        mulMat(m) {
            const r = Array.from({ length: rows }, () => Array.from({ length: m.cols }, () => C.zero()));
            for (let i = 0; i < rows; i++) {
        ...
        dagger() {
            const r = Array.from({ length: cols }, (_, j) => Array.from({ length: rows }, (_, i) => C.conj(data[i][j])));
        ...
        isUnitary(eps = 1e-8) {
            ...
            for (let i = 0; i < rows; i++) {
                for (let j = 0; j < cols; j++) {
                    ...
                    if (!C.eq(uu.data[i][j], expected, eps))
                        return false;
                }
            }
            return true;
...
export const Matrix2 = (data) => createMatrix(2, 2, data);
export const Matrix4 = (data) => createMatrix(4, 4, data);
export const Matrix8 = (data) => createMatrix(8, 8, data);
export function identity(n) {
    const data = Array.from({ length: n }, (_, i) => Array.from({ length: n }, (_, j) => (i === j ? C.one() : C.zero())));
...
export function kronMat(a, b) {
    const rows = a.rows * b.rows;
    const cols = a.cols * b.cols;
    const data = Array.from({ length: rows }, () => Array.from({ length: cols }, () => C.zero()));
...
export function applySingleQubitGate(state, n, target, gate) {
    const dim = 1 << n;
...
export function applyTwoQubitGate(state, n, control, target, gate) {
    if (control === target)
...
export function parseComplex(s) {
    ...
    if (!trimmed)
        return C.zero();
    if (/^[+-]?\d*\.?\d+$/.test(trimmed))
        return { re: parseFloat(trimmed), im: 0 };
