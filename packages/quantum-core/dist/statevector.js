...
export function stateLabel(index, n) {
...
export function blochCoordinates(state) {
    if (state.n !== 1)
        throw new Error('Bloch coordinates require one qubit');
...
export function measureQubitInBasis(state, qubitIndex, basis) {
    const n = state.n;
    const dim = 1 << n;
    ...
    const scale = 1 / Math.sqrt(norm2);
    for (let i = 0; i < dim; i++)
        newAmps[i] = C.scale(scale, newAmps[i]);
...
function transformQubitBasis(state, qubit, basis, toBasis) {
    if (basis === 'Z')
        return state;
    const H = basis === 'X';
...
export function singleQubitMeasurementProbabilities(alpha, beta, basis) {
...
export function isProductState(state) {
    if (state.n === 1)
        return true;
    if (state.n === 2) {
