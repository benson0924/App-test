...
import { Matrix2, Matrix4, Matrix8 } from './matrix.js';
...
export const Gates = {
    ...
    Toffoli: Matrix8([
        ...Array.from({ length: 6 }, (_, i) => Array.from({ length: 8 }, (_, j) => (i === j ? C.one() : C.zero()))),
        [C.zero(), C.zero(), C.zero(), C.zero(), C.zero(), C.zero(), C.zero(), C.one()],
        [C.zero(), C.zero(), C.zero(), C.zero(), C.zero(), C.zero(), C.one(), C.zero()],
    ]),
};
export function getSingleQubitGate(name, param) {
    switch (name) {
